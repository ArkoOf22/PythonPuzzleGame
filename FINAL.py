...
def start(inv):
	...
	if choice =='1':
		...
		if 'key' in inv:
			os.system('clear')
			mixer.init()
			mixer.music.load('doorclose.mp3')
			mixer.music.play()
			time.sleep(3)
...
def inside(inv):
	...
	engine.runAndWait()   
	mixer.init()
	mixer.music.load('footsteps.mp3')
	mixer.music.play()
	time.sleep(7)
	print('Trying to open the door...\n\n')
	mixer.init()
...
engine.runAndWait()
time.sleep(1)
print(colored("Welcome to the new Game",'blue'))
engine.say('Let me present to you')
engine.runAndWait()
time.sleep(1.5)
